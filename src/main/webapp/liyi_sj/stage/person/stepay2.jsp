...
<head>	
	...
	<link href="../AmazeUI-2.4.2/assets/css/admin.css" rel="stylesheet" type="text/css">
		...
		<link href="../personcss/stepstyle.css" rel="stylesheet" type="text/css">
			...
			<div class="col-main">
				<div class="main-wrap">
					...
					<div class="am-cf am-padding">
						<div class="am-fl am-cf"><strong class="am-text-danger am-text-lg">支付密码</strong></div>
					</div>
					<hr/>
					<!--进度条-->
					...
					<form class="am-form am-form-horizontal">
						...
						<div class="am-form-group code">
							...
							<a class="btn" href="javascript:void(0);" onclick="yanz()" id="sendMobileCode">
								<div onclick="" class="am-btn am-btn-danger">验证码</div>
							</a>
						</div>
						<div class="am-form-group">
							<label for="user-password"  class="am-form-label">支付密码</label>
							<div class="am-form-content">
								<input type="password" maxlength="6" id="user-password" placeholder="6位数字">
							</div>
						</div>
						<div class="am-form-group">
							<label for="user-confirm-password" class="am-form-label">确认密码</label>
							<div class="am-form-content">
								<input type="password" maxlength="6" id="user-confirm-password" placeholder="请再次输入上面的密码">
							</div>
						</div>
						<div class="info-btn">
							<div onclick="tijiao()" class="am-btn am-btn-danger">保存修改</div>
						</div>
...
<aside class="menu">
	...
	<script>
        ...
        if(userid!='') {
			...
			$("#user-phone").html(data.obj.phone);
            });
        ...
        var yanma=null;
        function yanz() {
			var yanzobj=new Object();
			yanzobj.phone=$("#user-phone").html();
            doData("sj_commodity/duanxing",yanzobj,function (data) {
                yanma=data.yanzma;
            })
		...
		function tijiao() {
            var obj=new Object();
            var yanzma=$("#user-code").val();
            obj.id=userid;
            obj.paypassword=""+$("#user-password").val();
			if (obj.paypassword!=""&&$("#user-password").val()!="") {
                if (yanzma == yanma) {
                    if ($("#user-confirm-password").val() == $("#user-password").val()) {
                        doData("sj_information/edit", obj, function (data) {
                            if (data.code > 0) {
                                alert("恭喜您,支付密码修设置成功");
                                window.location.reload();
                            }
